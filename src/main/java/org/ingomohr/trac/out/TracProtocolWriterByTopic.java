...
public class TracProtocolWriterByTopic {
	...
	private void writeEntries(PrintStream out, List<Entry> entries) {
		...
		out.println("Total time spent: " + getInfoMinutes(allMinutes, allMinutes) + "min");
		out.println();
	...
	private String toEntryLine(Entry entry, int allMinutes) {
		...
		builder.append(getInfoMinutes(entry.totalMinutesSpent, allMinutes));
		builder.append(" ");
	...
	private String getInfoMinutes(int totalMinutesSpent, int allMinutes) {
		final int maxLength = String.valueOf(allMinutes).length();

		final String actual = String.valueOf(totalMinutesSpent);

		StringBuilder builder = new StringBuilder();

		int numThousands = 0;
		for (int i = actual.length() - 1; i >= 0; i--) {
			builder.insert(0, actual.charAt(i));
			numThousands++;

			if (i > 0 && numThousands >= 3) {
				builder.insert(0, ".");
			}
		}

		while (maxLength > builder.length()) {
			builder.insert(0, " ");
		}
